...
import Image from "next/image"; // ✅ Import correctly at the top
import { expandedQuestions } from "./questions";

// ✅ Define the image-fetching function properly
const fetchRandomImage = (query) =>
  `https://loremflickr.com/400/300/${query}`;
  ...
  if (showResults) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-pink-500 to-purple-700 text-white p-6">
        ...
        <button
          className="mt-4 px-6 py-2 bg-white text-pink-500 font-bold rounded-full shadow-lg"
          onClick={() => window.location.reload()}
        >
          Restart Quiz
        </button>
        <footer className="mt-6 text-sm">
          Made with ❤️ for Avani for her to study for her exam and not find reasons to not study
        </footer>
      </div>
    );
  }
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-pink-500 to-purple-700 text-white p-6">
      ...
      <motion.div key={currentIndex} initial={{ opacity: 0, x: 100 }} animate={{ opacity: 1, x: 0 }}>
        <div className="p-6 w-96 bg-white text-black rounded-lg shadow-lg">
          {/* ✅ Fixed Image Import - No Syntax Errors */}
          <Image
          ...
          {questions[currentIndex]?.options.map((option, index) => (
            <button
              key={index}
              className={`w-full mb-2 p-2 rounded-lg shadow-md transition duration-200 ${
                selectedAnswer === option ? "bg-gray-300" : "bg-pink-500 text-white hover:bg-pink-600"
              }`}
